...
function GetFilesPurchased( $file_content, $bHTML )
{
	...
	for( $i=1; $i<1000; $i++ ) 
	{
		...
		if( $pdFileUrl == "" ) 
			// #1163 si le produit $i n'est pas numérique alors il ne faut pas stopper la recherche mais continuer avec les autres
			//break;  
			continue;
...
function SendOrderEmail( $orderid, $emailtitle, $emailmessage ) {
	...
	if( $cust_email != "" ) {
		...
		if( $emailtitle != "" && $emailmessage != "" ) {
			if( $MERCHANT_BCC )
				$cust_email .= ";".$MERCHANT_FROM; 
			SendPhpMail( $mail_account, $cust_email, $emailtitle, $emailmessage );
		}
	}
}
