<?php
	...
	$VO_CTR = "aa7cbf3e8f427264e46af7ba5273063b";					
	$MAX_PERIOD = -1;				
	$ORDER_EMAIL_ADD_URL = false;		
	$ACCOUNTS_URL = "";
	$mail_account = ["username"=>"minombre@miproveedor.com"];	
	$COOKIE_ACCOUNT_INF = "3570-account_inf";
			...
			for( $i=0; $i < count( $items ); $i++ ) {
				if(array_key_exists($items[$i]->twref, $json_realtime_stock)) {
					...
					if (gettype($stock) !== 'integer') {
					    if (is_string($stock) && is_numeric($stock) && (string)(int)$stock === $stock) {
					        $stock = (int)$stock;
					    } else {
	...
	$err_code = SendPhpMail( $mail_account, $MERCHANT_TO, $v_subject, $otxt );
	if( $err_code <> "") 
		$result .= "ERR_DEMO ($err_code)";
	if( $cemail != "" ) {
		sleep(1);
		...
		$cinst .= ALTBODY_SEP_TAG . $otxt;
		$to = $cemail;
		if( $MERCHANT_BCC )
			$to .= ";".$MERCHANT_TO;
		$err_code = SendPhpMail( $mail_account, $to, $c_subject, $cinst );
		if( $err_code <> "") 
			$result .= " ($err_code)";
	}
	echo $result;
